...
fs.readFile('config.angular.json', function (err, data) {
    ...
    const LAZY_ARRAY =  dirs(LOCAL_EXT_PATH).map(p => {
        ...
        if (fs.existsSync(CSS_ASSET)) {
            fse.copy(CSS_ASSET, LOCAL_ASSETS_PATH, function (err) {if (err){return console.error(err)}});
        }
        if (fs.existsSync(IMG_ASSET)) {
            fse.copy(IMG_ASSET, LOCAL_ASSETS_IMG_PATH, function (err) {if (err){return console.error(err)}});
        }
